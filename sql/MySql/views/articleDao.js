...
let Handle = {
     ...
     * 根据目录ID删除文章
     */
    deleteOneByPids(pids, callback) {
        let sql = `DELETE FROM ${tableName} WHERE pid in (${pids})`;
        pool.getConnection((err, conn) => {
     ...
     */
    updateOneRead(id, read, callback) {
        let sql = `UPDATE ${tableName} SET  readTime = ? WHERE id=?`;
        pool.getConnection((err, conn) => {
    ...
    queryLike(content, start, end, callback) {
        let sql = `
            SELECT 
            id,pid,articleName,alertDate,tags,uId,good,canCross
            FROM ${tableName} 
    ...
    /**
     * 根据content,模糊查询文章的标题或标签总数量。
     */
    queryLikeSum(content, callback) {
        let sql = `
                SELECT 
                COUNT(*)
                FROM ${tableName} 
     ...
     * 根据用户id和Content,分页模糊查询文章的标题或标签。
     */
    queryLikeByUIdSum(uId, content,callback) {
        let sql = `
                SELECT 
                COUNT(*)
                FROM ${tableName} 
                ...
                `;
        pool.getConnection((err, conn) => {
            conn.query(sql, [Number(uId), `%${content}%`], (err, data) => {
                callback(err, data);
            })
            conn.release();
        });
